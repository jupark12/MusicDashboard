  ...
  return (
    <div className="UploadModal">
        ...
        {currentSlide === 1 ? ( // First slide: Audio file selection
          ...
          <div className="flex flex-col items-center h-full">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col items-center w-full h-full">
              {formData.cover ? (
                <img src={formData.cover} alt="Cover" className="h-32 w-32 mb-4 rounded-full" />
              ) : (
              ...
              <div className="flex justify-between w-full mt-auto">
                <button className="mt-4" type="button" onClick={goToPreviousSlide}><FaChevronLeft size={16} /></button>
                <button className="mt-4 ml-auto" type="submit">Add Song</button>
              </div>
            </form>
          </div>
        )}
      ...
      <div className="error w-full justify-center flex mt-4">
        <p className={`error-text ${errorMessage ? "visible" : ""}`}>{errorMessage}</p>
      </div>
    </div>
  );
};
